...
import { CountrySpendData, DemographicChartData } from '@/types/demographicData';
import { FilterState } from '@/pages/Dashboard';
import { format, parseISO, isAfter, isBefore, startOfDay } from 'date-fns';

interface CountrySpendChartProps {
  data: CountrySpendData[];
  filters: FilterState;
}
...
export const CountrySpendChart: React.FC<CountrySpendChartProps> = ({ data, filters }) => {
  ...
  const chartData = useMemo((): DemographicChartData[] => {
    ...
    // Group by country and sum spend
    const countrySpend = filteredData.reduce((acc, item) => {
      if (!acc[item.country]) {
        acc[item.country] = 0;
      }
      acc[item.country] += item.spend;
      return acc;
    }, {} as Record<string, number>);

    const totalSpend = Object.values(countrySpend).reduce((sum, spend) => sum + spend, 0);
